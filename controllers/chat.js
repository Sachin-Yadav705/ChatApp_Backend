...
const getMyChat = TryCatch(async (req, res, next) => {
  ...
  const transformedChats = chats.map(({ _id, name, members, groupChat }) => {
    ...
    return {
      ...
      // members: members
      //   .filter((i) => i._id.toString() !== req.user.toString())
      //   .map((i) => i._id),
    };
  });
...
// Get The list of group which i have created
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length > 50)
    return next(new ErrorHandler("You can't add more than 50 members", 400));
  await chat.save();
...
export { newGroupChat, getMyChat, getMyGroup, addMembers, removeMember };
